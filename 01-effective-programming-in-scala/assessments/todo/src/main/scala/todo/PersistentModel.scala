...
object PersistentModel extends Model:
  ...
  def update(id: Id)(f: Task => Task): Option[Task] =
    var tasks = mutable.LinkedHashMap.from(loadTasks().toMap)
    val task = tasks.updateWith(id)(opt => opt.map(f))
  ...
  def delete(id: Id): Boolean =
    var found = false
    val tasks = loadTasks()
    if tasks.toMap.get(id) != None then
      saveTasks(Tasks(tasks.toList.filter(task => task._1 != id)))
      found = true
    found
  ...
  def tasks(tag: Tag): Tasks =
    Tasks(
      loadTasks().toList.filter(task =>
        task._2.tags.find(x => x == tag) != None
      )
    )

  def complete(id: Id): Option[Task] =
    val task = loadTasks().toMap.get(id).get.complete
    update(id)(_ => task)

  def tags: Tags =
    Tags(loadTasks().toList.flatMap(task => task._2.tags).toSet.toList)

  /** Delete the tasks and id files if they exist.
    */
  def clear(): Unit =
    if Files.exists(tasksPath) then new File(tasksPath.toUri).delete()
    if Files.exists(idPath) then new File(idPath.toUri).delete()
    ()
